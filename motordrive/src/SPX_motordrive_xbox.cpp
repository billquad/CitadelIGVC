...
class SubscribeAndPublish
{
	...
	void joyCallback(const sensor_msgs::Joy::ConstPtr& joy)
	{
		...
		// pressing B
			...
			if ( speed.data > 0.0 ){
                                duty_cycle_L.data = 118.0*velocity_L.data+137.0;
                                duty_cycle_R.data = 118.0*velocity_R.data+137.0;
			} else {
                                duty_cycle_L.data = 127.0*velocity_L.data+127.0;
                                duty_cycle_R.data = 127.0*velocity_R.data+127.0;
			}
