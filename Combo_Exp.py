...
while True:
    ...
    frame = cv2.GaussianBlur(orig_frame, (9, 9), 10000)
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HLS) #Converts to frame to HSL
    low_white = np.array([0, 170, 0])       #These values are in HSL
    ...
    cv2.circle(frame, (240, 225), 5, (0, 0, 255), -1)
    cv2.circle(frame, (1040, 225), 5, (0, 0, 255), -1)
    cv2.circle(frame, (5, 715), 5, (0, 0, 255), -1)
    cv2.circle(frame, (1275, 715), 5, (0, 0, 255), -1)    
    pts1 = np.float32([[240, 225], [1040, 225], [5, 715], [1275, 715]])
